...
struct MockECUSettings {
    ...
    var echo = false
    var spaces = false
    var vinNumber = "1G4AW69N2DH524774"
}
...
class MOCKComm: CommProtocol {
    ...
    func sendCommand(_ command: String, retries: Int = 3) async throws -> [String] {
        ...
        if prefix == "01" || prefix == "06" || prefix == "09" {
            ...
            var response = {
                switch action {
                    ...
                    case "E0":
                        ...
                        return ["OK"]
                    case "S0":
                        ecuSettings.spaces = false
                        return ["OK"]
                    case "S1":
                        ecuSettings.spaces = true
                        return ["OK"]
                    case "DPN":
                        return ["06"]
                    case "RV":
            ...
            return ecuSettings.spaces ? response : response.map { $0.replacingOccurrences(of: " ", with: "") }
...
extension OBDCommand {
    static func mockResponse(forCommand command: String) -> String? {
        guard let obd2Command = from(command: command) else {
